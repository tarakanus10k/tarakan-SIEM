...
class MultilineUnitReader(BaseReader):

    def __init__(self, lines: int, encoding: str = "utf-8") -> None:
    ...
    def read(self, data: Union[bytes, str]) -> list[str]:
        ...
        messages: list[str] = []
        for line in lines:
            self._current.append(line.rstrip("\r"))

            if len(self._current) >= self._lines:
                messages.append("\n".join(self._current))
                self._current = []
